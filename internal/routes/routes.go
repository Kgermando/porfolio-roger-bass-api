...
func Setup(app *fiber.App, db *gorm.DB) {
	...
	admin.Put("/articles/:id/publish", articleHandler.TogglePublish)
	admin.Delete("/articles/:id", articleHandler.Delete)

	// Analytics
	admin.Get("/analytics/stats", analyticsHandler.AdminStats)

	// Media upload → Backblaze B2
	admin.Post("/upload", uploadHandler.UploadImage)
	admin.Post("/upload/video", uploadHandler.UploadVideo)
}
